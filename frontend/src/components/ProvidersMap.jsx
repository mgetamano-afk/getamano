...
const buildMarkerIcon = (color = "#2F9D94", isLatino = false, isAmerican = false) => {
  const accent = isLatino ? "#025F67" : isAmerican ? "#185FA5" : color;
  const svg = `
    <svg xmlns='http://www.w3.org/2000/svg' width='34' height='44' viewBox='0 0 34 44'>
      <defs>
        <filter id='s' x='-20%' y='-20%' width='140%' height='140%'>
          <feDropShadow dx='0' dy='1.5' stdDeviation='1.2' flood-color='#063154' flood-opacity='0.35'/>
        </filter>
      </defs>
      <path filter='url(#s)' fill='${accent}' stroke='white' stroke-width='2'
            d='M17 1 C 8 1 2 8 2 16 c 0 11 15 26 15 26 s 15 -15 15 -26 c 0 -8 -6 -15 -15 -15 z'/>
      <circle cx='17' cy='16' r='5' fill='white'/>
    </svg>`;
  return L.divIcon({
    className: "getamano-marker",
    html: svg,
    iconSize: [34, 44],
    iconAnchor: [17, 42],
    popupAnchor: [0, -38],
...
export default function ProvidersMap({ providers, loading }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border" style={{ borderColor: "#BCC5CC" }} data-testid="providers-map-container">
      <div className="relative" style={{ height: "min(72vh, 720px)" }}>
        ...
        >
          ...
          {items.map(p => (
            <Marker
              ...
              icon={buildMarkerIcon("#2F9D94", p.owner_identity === "latino", p.owner_identity === "american")}
              eventHandlers={{ click: () => setActive(p.provider_id) }}
            >
              <Popup>
        ...
        {/* Tiny attribution overlay (subtle) */}
        {items.length > 0 && (
      ...
      <style>{`
        ...
        .getamano-marker { background: transparent !important; border: none !important; }
      `}</style>
      {/* active is used internally to highlight selection state; placeholder line keeps lint happy */}
